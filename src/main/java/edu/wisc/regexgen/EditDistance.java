...
public class EditDistance {
	...
	private static Pair<Integer, LinkedList<CharPred>> moveFromSToTGivenC(int T, int S, Character c, SFA<CharPred, Character> templ) {
		//System.out.println(String.format("From %d to %d given [%c]", T,S,c));
		Triple<Integer, Integer, Character> lookUp = new Triple<>(T, S, c);
		if (vStorage.containsKey(lookUp)) {
			return vStorage.get(lookUp);
		}
		Pair<Integer, LinkedList<CharPred>> pResult = shortestStrFromTToS(templ.stateCount() - 1, T, S, templ);
	 ...
	 */
	private static Pair<Integer, LinkedList<CharPred>> shortestStrFromTToS(int k, int T, int S, SFA<CharPred, Character> templ) {
		...
		if (k == 0) {
			...
			for (Move<CharPred, Character> q : arcs) {
				if (q.to == S) {
					if (q.isEpsilonTransition()) {
						LinkedList<CharPred> l = new LinkedList<CharPred>();
						Pair<Integer, LinkedList<CharPred>> res = new Pair<Integer, LinkedList<CharPred>>(0, l);
						pStorage.put(lookUp, res);
						return res;
					}
					SFAInputMove<CharPred, Character> curr = (SFAInputMove<CharPred, Character>) q;
		...
		Pair<Integer, LinkedList<CharPred>> term1 = shortestStrFromTToS(k - 1, T, S, templ);
		Pair<Integer, LinkedList<CharPred>> term2 = shortestStrFromTToS(k - 1, T, k, templ);
		Pair<Integer, LinkedList<CharPred>> term3 = shortestStrFromTToS(k - 1, k, S, templ);
